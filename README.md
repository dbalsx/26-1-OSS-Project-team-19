...
4. 설치가 끝나고 필수 파일(`best.pt` 모델 가중치 파일 및 `test_video1.mp4` 등 시연 영상)이 폴더 내에 잘 존재하는지 확인한 후, 터미널에 `streamlit run app.py` 명령어를 입력하여 보행 보조 시스템을 실행합니다. 명령어를 입력하면 자동으로 웹 브라우저가 열리며 시스템에 접속되어 바로 시연을 진행할 수 있습니다.
